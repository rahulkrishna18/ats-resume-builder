const checks = [
  ...
  },
  {
    ...
    points: 20,
    test: (d) =>
      (d.skills.technical || d.skills.soft || d.skills.languages || d.skills.certifications).trim()
        .length > 10,
  ...
  },
  {
    ...
    points: 20,
    test: (d) =>
      d.experience.some((e) => e.company && e.role && e.bullets.some((b) => b.trim().length > 10)),
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
              {score >= 80
                ...
                : score >= 55
                  ? 'Good start — a few improvements will boost your chances.'
        ...
        <div className="space-y-1.5">
          {results.map((r) => (
            <div key={r.id} className="flex items-start gap-2.5">
              <div
                className={`mt-0.5 flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${r.passed ? 'bg-emerald-100' : 'bg-slate-100'}`}
              >
                {r.passed ? (
                  <svg
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`text-xs font-medium ${r.passed ? 'text-slate-700' : 'text-slate-500 line-through'}`}
                >
                  {r.label}
                </p>
                <span className={`text-xs ${r.passed ? 'text-emerald-600' : 'text-slate-400'}`}>
                  +{r.points} pts
                </span>
              </div>
            </div>
