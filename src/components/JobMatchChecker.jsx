...
const CATEGORY_META = {
  hardSkills: { label: 'Technical / Hard Skills', color: 'blue', bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-700', pill: 'bg-blue-100 text-blue-700' },
  softSkills: { label: 'Soft Skills', color: 'violet', bg: 'bg-violet-50', border: 'border-violet-200', text: 'text-violet-700', pill: 'bg-violet-100 text-violet-700' },
  experience: { label: 'Role / Experience', color: 'amber', bg: 'bg-amber-50', border: 'border-amber-200', text: 'text-amber-700', pill: 'bg-amber-100 text-amber-700' },
  education:  { label: 'Education', color: 'emerald', bg: 'bg-emerald-50', border: 'border-emerald-200', text: 'text-emerald-700', pill: 'bg-emerald-100 text-emerald-700' },
};
...
function ScoreRing({ score, size = 120 }) {
  ...
  const labelColor = score >= 75 ? 'text-emerald-600' : score >= 50 ? 'text-amber-500' : 'text-red-500';
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="text-xs text-slate-500">{data.matched}/{data.total} keywords</span>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 pt-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Upload your resume and paste a job description to see how well your resume matches the role — fully in your browser, no data sent anywhere.
          </p>
        </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  {jobDescription && (
                    <button onClick={() => setJobDescription('')} className="text-xs text-slate-400 hover:text-slate-600 transition-colors">Clear</button>
                  )}
                </div>
              </div>
            </div>
                ...
                {activeTab === 'upload' ? (
                  <div>
                    <label
                      ...
                      className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl cursor-pointer transition-all h-40 ${
                        ...
                        resumeFileName ? 'border-emerald-300 bg-emerald-50' :
                    ...
                    >
                      {loading ? (
                        ...
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-emerald-700">{resumeFileName}</span>
                          <span className="text-xs text-emerald-600">{resumeText.length.toLocaleString()} characters extracted</span>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center gap-2 px-4 text-center">
                          ...
                          <span className="text-xs text-slate-400">PDF or TXT — parsed entirely in your browser</span>
                        </div>
                      )}
                    </label>
                    <input
                      id="resume-upload" type="file" accept=".pdf,.txt" className="hidden"
                      onChange={e => processFile(e.target.files[0])}
                    />
                    {resumeFileName && (
                      <button onClick={() => { setResumeFileName(''); setResumeText(''); }}
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      {resumeText && <button onClick={() => setResumeText('')} className="text-xs text-slate-400 hover:text-slate-600 transition-colors">Clear</button>}
                    </div>
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs text-slate-400">
                {result.totalKeywords} keywords extracted from JD · {result.matchedKeywords.length} found in resume
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="col-span-1 space-y-4">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                  <p className="text-xs font-bold text-amber-800 uppercase tracking-wide mb-3">Top Recommendations</p>
                  <div className="space-y-2">
                    {result.score < 40 && (
                      <p className="text-xs text-amber-700 flex gap-1.5"><span className="flex-shrink-0">▸</span>Your resume has very low keyword overlap. Review the job requirements and tailor your experience section directly to the role.</p>
                    )}
                    {result.categoryScores.hardSkills?.total > 0 && result.categoryScores.hardSkills?.score < 60 && (
                      <p className="text-xs text-amber-700 flex gap-1.5"><span className="flex-shrink-0">▸</span>You're missing key technical skills. Add any relevant tools you've used to your Skills section, even if briefly.</p>
                    )}
                    {result.missingKeywords.length > 0 && (
                      <p className="text-xs text-amber-700 flex gap-1.5"><span className="flex-shrink-0">▸</span>Weave {Math.min(5, result.missingKeywords.length)} missing keywords naturally into your summary or experience bullet points.</p>
                    )}
                    {result.score >= 75 && (
                      <p className="text-xs text-amber-700 flex gap-1.5"><span className="flex-shrink-0">▸</span>Strong match! Ensure your bullet points quantify achievements with numbers (%, $, team size).</p>
                    )}
                    {result.categoryScores.softSkills?.total > 0 && result.categoryScores.softSkills?.score < 50 && (
                      <p className="text-xs text-amber-700 flex gap-1.5"><span className="flex-shrink-0">▸</span>Include soft skills mentioned in the JD (e.g. "leadership", "collaboration") in your summary or experience bullets.</p>
                    )}
                  </div>
                </div>
              </div>
                        ...
                        return (
                          <div key={cat}>
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-xs text-slate-400">{matched}/{items.length} matched</span>
                            </div>
                            <div className="flex flex-wrap gap-1.5">
                              {items.map(r => <KeywordPill key={r.keyword} keyword={r.keyword} found={r.found} />)}
                            </div>
                          </div>
                        );
                      })}
                  ...
                  {activeSection === 'matched' && (
                    <div>
                      {result.matchedKeywords.length === 0 ? (
                        ...
                        <div>
                          <p className="text-xs text-slate-500 mb-3">These keywords from the JD appear in your resume:</p>
                          <div className="flex flex-wrap gap-1.5">
                            {result.matchedKeywords.map(r => <KeywordPill key={r.keyword} keyword={r.keyword} found={true} />)}
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      {result.missingKeywords.length === 0 ? (
                        <div className="flex flex-col items-center py-12 text-emerald-500">
                          ...
                          <p className="text-sm font-medium">All keywords matched! Perfect score.</p>
                        </div>
                      ) : (
                        <div>
                          <p className="text-xs text-slate-500 mb-3">
                            These keywords appear in the JD but are <span className="font-semibold text-red-600">not found</span> in your resume. Consider adding the relevant ones:
                          </p>
                          <div className="space-y-4">
                            {Object.entries(result.byCategory).map(([cat, items]) => {
                              ...
                              return (
                                <div key={cat}>
                                  <p className={`text-xs font-bold uppercase tracking-wider mb-1.5 ${meta.text}`}>{meta.label}</p>
                                  <div className="flex flex-wrap gap-1.5">
                                    {missing.map(r => <KeywordPill key={r.keyword} keyword={r.keyword} found={false} />)}
                                  </div>
                                </div>
                              );
                            })}
